...
let handler = async (m, {conn, args, usedPrefix, command}) => {
    ...
    const videoApis = [
        ...
        {
            url: () => fetch(`https://api.siputzx.my.id/api/d/ytmp4?url=${args}`).then(res => res.json()),
            extract: (data) => ({data: data.dl, isDirect: false})
        },
        {
            url: () => fetch(`https://api.neoxr.eu/api/youtube?url=${args}&type=video&quality=720p&apikey=GataDios`).then(res => res.json()),
            extract: (data) => ({data: data.data.url, isDirect: false})
        },
        {
            url: () => fetch(`${global.APIs.fgmods.url}/downloader/ytmp4?url=${args}&apikey=${global.APIs.fgmods.key}`).then(res => res.json()),
            extract: (data) => ({data: data.result.dl_url, isDirect: false})
        },
        {
            url: () => fetch(`${apis}/download/ytmp4?url=${args}`).then(res => res.json()),
            extract: (data) => ({data: data.status ? data.data.download.url : null, isDirect: false})
        },
        {
            url: () => fetch(`https://exonity.tech/api/ytdlp2-faster?apikey=adminsepuh&url=${args}`).then(res => res.json()),
    ...
    if (videoData) {
        ...
        if (fileSize > LimitVid) {
            ...
            await conn.sendMessage(m.chat, {
                video: isDirect ? videoData : {url: videoData},
                thumbnail: await fetch((await yts(youtubeLink)).videos[0].thumbnail), ...messageOptions
            }, {quoted: m});
        }
    } else {
