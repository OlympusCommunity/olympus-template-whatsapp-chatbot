...
`
            ...
            const idmessage = await conn.sendMessage(m.chat, {
                image: glx_menu,
                caption: str.trim()
            }, {quoted: selo1234});
...
*_${usedPrefix}glx bau_* - Para ver sus items minados.`, edit: messageId.key
        ...
        function valorFormatado(valor) {
            const valorFormatado = (valor).toLocaleString(data.perfil.idioma, {
                style: 'currency',
            ...
            for (let i = 0; i < db.user_cadastrado.username.length; i++) {
                ...
                if (db.user_cadastrado.username[i].username === alvo) {
                    ...
                    if (db1.perfil.defesa.forca >= data.perfil.ataque.forcaAtaque.ataque) {
                        ...
                        conn.sendMessage(db1.perfil.id, {
                            text: `_Prepare su defensa 🛡️, en 10 segundos, serás atacado por *@${number2}!*_`,
                            mentions: [data.perfil.id]
                        })
...
Perdió: ${db1.perfil.defesa.ataque} Puntos
  ...
  *💡 Consejo:* _Si su defensa esta perdiendo muchos puntos, compre más armas *(glx comprar)* o mine más minerales *(glx miner)* para aumentar su fuerza ._
                        ...
                        `
                            ...
                            conn.sendMessage(id, {
                                text: str,
                                mentions: [db.user_cadastrado.username[i].id, db.user_cadastrado.username[i].id]
                            })
                        }, 5000)
                        break;
                    }
                    ...
                    conn.sendMessage(db.user_cadastrado.username[i].id, {
                        text: str,
...
`, mentions: [db.user_cadastrado.username[i].id]
                        ...
                        conn.sendMessage(db.user_cadastrado.username[i].id, {
                            text: `@${number} que triste! 😭\n\n*⚔️ Tu defensa falló ⚔️* \n\n> _Hay daños en tus instalaciones._`,
                            mentions: [db.user_cadastrado.username[i].id]
                        })
                    }, 10000)
