...
let handler = async (m, {text, conn, args, usedPrefix, command}) => {
    ...
    const audioApis = [
        ...
        {
            url: () => fetch(`https://api.neoxr.eu/api/youtube?url=${args}&type=audio&quality=128kbps&apikey=GataDios`).then(res => res.json()),
            extract: (data) => ({data: data.data.url, isDirect: false})
        },
        {
            url: () => fetch(`https://api.fgmods.xyz/api/downloader/ytmp4?url=${args}&apikey=${fgkeysapi}`).then(res => res.json()),
            extract: (data) => ({data: data.result.dl_url, isDirect: false})
        },
        {
            url: () => fetch(`https://api.siputzx.my.id/api/d/ytmp4?url=${yt_play[0].url}`).then(res => res.json()),
        ...
        {
            url: () => fetch(`${apis}/download/ytmp3?url=${yt_play[0].url}`).then(res => res.json()),
            extract: (data) => ({data: data.result?.link, isDirect: false})
        },
        {
            url: () => fetch(`https://api.dorratz.com/v3/ytdl?url=${encodeURIComponent(yt_play[0].url)}`).then(res => res.json()),
            extract: (data) => {
