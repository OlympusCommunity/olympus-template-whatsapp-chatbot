...
async function handler(m, {conn, args, usedPrefix, command}) {
    ...
    if (confirmation[m.sender]) return conn.sendMessage(m.chat, {
...
`.trim();
    ...
    if (!(who in global.db.data.users)) return conn.sendMessage(m.chat, {
        text: `${tradutor.texto4[0]} ${who} ${tradutor.texto4[1]}`,
        mentions: [m.sender]
    }, {quoted: m});
    if (user[type] * 1 < count) return conn.sendMessage(m.chat, {
        text: `${tradutor.texto5[0]} ${type} ${tradutor.texto5[1]}`,
        mentions: [m.sender]
    }, {quoted: m});
...
${tradutor.texto6[4]}`.trim();
    ...
    confirmation[m.sender] = {
        ...
        timeout: setTimeout(() => (conn.sendMessage(m.chat, {
            text: '*[❗] Se acabó el tiempo, no se obtuvo respuesta. Transferencia cancelada.*',
            mentions: [m.sender]
        }, {quoted: m}), delete confirmation[m.sender]), 60 * 1000)
    };
}
    ...
    if (/^Si|si$/i.test(m.text)) {
        ...
        if (previous > user[type] * 1 && _previous < _user[type] * 1) {
            conn.sendMessage(m.chat, {
                text: `${tradutor.texto8} ${count} ${type} a @${(to || '').replace(/@s\.whatsapp\.net/g, '')}*`,
                mentions: [to]
            }, {quoted: m});
        } else {
            user[type] = previous;
            ...
            conn.sendMessage(m.chat, {
                text: `${tradutor.texto9} ${count} ${type} a @${(to || '').replace(/@s\.whatsapp\.net/g, '')}*`,
                mentions: [to]
            }, {quoted: m});
        }
        clearTimeout(timeout);
